...
define( 'NOTIFIER_THEME_NAME', 'PowerMag' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', 'powermag' ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://powermag.djwd.me/wp-content/themes/powermag/admin/pm_notifier.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		$theme = wp_get_theme();
		$theme_data = $theme->Version;
		
		/// END DJWD ///
		
		
		if( (float)$xml->latest > (float)$theme_data /*djwd*/) { // Compare current theme version with the remote XML version
...
function update_notifier() { 
	...
	<div class="wrap">
	    ...
	    <div id="message" class="updated below-h2"><p><strong>There is a new version of the <?php echo NOTIFIER_THEME_NAME; ?> theme available.</strong> You have version <?php echo $theme_data /*djwd*/; ?> installed. Update to version <?php echo $xml->latest; ?>.</p></div>
