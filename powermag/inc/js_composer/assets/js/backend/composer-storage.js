...
(function ($) {
    ...
    Storage.prototype = {
        ...
        wrapData: function(content) {
            var tags = _.keys(vc.map).join('|'),
                reg_m = new RegExp('(\\[\\[?\\/?['+ _.keys(vc.map).join('|') + ']+(?!\w-))', ''),// new RegExp('(\\[\\[?\\/?' + tags + '|\\]\\]?)', 'gi'),
                reg = this.regexp_split(tags),
                w_content = '',
                not_shortcodes = content.trim().split(reg);
            _.each(not_shortcodes, function(string, index){
                w_content += string;
            });
            return content;
        },
        ...
        regexp_split: _.memoize(function() {
            var tags =_.keys(vc.map).join('|');
